...
public class Mailer {
	private ArrayList<Integer> idlesList = new ArrayList<Integer>();
	...
	public void send(int reciever, Message m) {
		...
		synchronized(idlesList) {
			if(permission == false) {
				if(idlesList.contains(reciever) && (m.getHeader() == "Normal" || m.getHeader() == "BT")) {
					idlesList.remove(idlesList.indexOf(reciever));									
				}
			}
		}
	}
	...
	public ArrayList<Integer> getIdlesList() {
		return idlesList;
	}
