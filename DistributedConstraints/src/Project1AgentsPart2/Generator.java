...
public class Generator {
	...
	float p1; // the probability two agents are constrained.
	float p2; // the probability two values are constrained.
	...
	public Generator(int n, int k, float p1, float p2) {
		this.n = n;
		this.k = k;
		this.p1 = p1;
		this.p2 = p2;
	...
	public void constrainedWithWho(Agent agent) { // This function checks if the agents are constrained, and if they are, it counts the amount of values constrained (amount of 1's in the k*k matrix).
		for (int index = 0; index <= this.n - 1; index++) { 
			if (this.constraints[agent.getId()][index] != null) {  // if there is a k*k matrix between 2 agents.
				int counter = 0; // counts the amount of 1's.
				for (int row = 0; row < this.k ; row++) { // counts the amount of 1's between them.
					for (int col = 0; col < this.k ; col++) {
						if (this.constraints[agent.getId()][index][row][col] == 1) {
							counter++;
						}
					}
				}
				...
				agent.getConstraints()[agent.getId()][index] = counter; // also, adds the amount of 1's to the matrix of constraints values between the two agents. 
				// each agent holds such a matrix.
			}
		}
	}
