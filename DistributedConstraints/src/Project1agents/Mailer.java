...
public class Mailer {
	...
	public void send(int reciever, Message m) {
		synchronized (map) { 					// synchronizing the sending of messages => no sending to the same index at the same time.
	...
	public Message readOne(int reciever) {		
		if (this.map.get(reciever).size()==0) {	// returns null if the list of messages is empty.
			...
			synchronized(map) {					// synchronizing the reading of a message => no getting values to the same message by more then one agent at a time.
